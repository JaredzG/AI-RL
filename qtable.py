...
class Qtable:
  ...
  def choose_new_q_val(self, old_q_val, move_reward, next_q_vals, rl_method, chosen_policy, carrying, next_pos_cells, alpha, gamma):
    ...
    if rl_method == 'q_learning':
      ...
      return round(value.sarsa_value_function(old_q_val, move_reward, next_q_vals[chosen_action], alpha, gamma), 2)
    
  def update_qtable(self, position, move, layer, next_cell, next_actions, alpha, gamma, rl_method = 'q_learning', policy = 'p_random', carrying = False, next_pos_cells = None):
    ...
    new_q_val = self.choose_new_q_val(old_q_val, move_reward, next_q_vals, rl_method, policy, carrying, next_pos_cells, alpha, gamma)
  ...
  def find_q_vals(self, position, moves, carrying):
